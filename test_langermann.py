...
# DeJong5
dejong5_configs_df = pd.DataFrame()
for k in configs.keys():
    ...
    for _ in range(30):
        ...
        for i in range(number_of_epochs):
            population = clonalg(deJong5, population, x_size, x_range, selection_rate, clone_rate, mutation_range)
            
        population.sort(key=sort_func)
        bestOfRoundList.append(population[0][-1])

    dejong5_configs_df[k] = bestOfRoundList
    print("---------------------------")

dejong5_configs_df.to_csv("dejong_configs_clonalg.csv", sep=";", index=False)